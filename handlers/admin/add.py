...
cat_cb = CallbackData('category', 'id', 'action')
product_cb = CallbackData('product', 'id', 'action')

add_product = '➕ Добавить товар'
delete_category = '🗑️ Удалить категорию'
add_cat='➕ Добавить подкатегорию'
...
async def process_settings(message: Message):
    ...
    markup.add(InlineKeyboardButton(
        add_cat, callback_data='add_category'))
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.answer('Все добавленные подкатегорие в эту категорию.')
    await state.update_data(category_index=category_idx)
    await show_products1(query.message, products, category_idx)
...
async def delete_category_handler(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if 'category_index' in data.keys():
            ...
            db.query(
                'DELETE FROM categories WHERE tag IN (SELECT title FROM categori WHERE idx=?)', (idx,))
            db.query('DELETE FROM categori WHERE idx=?', (idx,))
...
@dp.message_handler(IsAdmin(), text=cancel_message, state=Categ.title)
...
@dp.message_handler(IsAdmin(), text=back_message, state=Categ.title)
async def process_title_back(message: Message, state: FSMContext):
    await process_add_product(message)

@dp.message_handler(IsAdmin(), state=Categ.title)
async def process_title(message: Message, state: FSMContext):
    ...
    await Categ.next()
    await message.answer('сохранить?', reply_markup=back_markup())

@dp.message_handler(IsAdmin(), text='Все правильно', state=Categ.confirm)
